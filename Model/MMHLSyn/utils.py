...
import torch
import numpy as np
from sklearn.metrics import roc_auc_score, precision_recall_curve, mean_squared_error, r2_score, mean_absolute_error
...
def get_MACCS(smiles):
    ...
    #fp = AllChem.GetMorganFingerprint(m, 2,1024)
    DataStructs.ConvertToNumpyArray(fp, arr)
    arr=np.append(arr,logp_m)
    return arr
...
def regression_metric(ytrue, ypred):
    ...
    rs = stats.spearmanr(ytrue,ypred)[0]
